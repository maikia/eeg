...
def make_dataset_from_sample():
    ...
    sel_data = mne.pick_types(info, meg='grad', eeg=False, stim=False, exclude=[])
    ...
    fwd = mne.pick_channels_forward(fwd, include=info_data['ch_names'], ordered=True)
...
def test_sparse_regressor_on_sample(make_dataset_from_sample, model, hl_max, params):
